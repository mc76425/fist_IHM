...
from widgets.sliders import SliderWidget, Canvas
from PyQt5.QtCore import Qt, QCoreApplication
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QLabel, QGridLayout, QWidget, QAction, QStatusBar, QToolBar, QPushButton, QComboBox, QMessageBox
...
# menus et toolbar
        ...
        self.dict_button_action = {0: self.set_color2,
                                   1: self.gomme_set,
                                   2: self.baguette_magique,
                                   3: self.pot_magique, }
        self.action_reint = QAction(
            QIcon("icones\\arrow-reinit.png"), "Réinitialiser", self)
        self.action2 = QAction(QIcon("icones\\palette.png"),
                               "Réinitialiser la palette", self)
        self.action3 = QAction(QIcon("flag-red.png"),
                               "Filtre rouge (fonctionnel)", self)
        ...
        self.action4 = QAction(
            QIcon("icones/flag-green.png"), "Filtre vert (fonctionnel)", self)
        self.action5 = QAction(QIcon("icones/flag-blue.png"),
                               "Filtre bleu (fonctionnel)", self)
        self.action6 = QAction(
            QIcon("icones/flag-negatif.png"), "Négatif", self)
        self.action7 = QAction(QIcon("icones/flag-grey.png"), "Gris", self)
        self.action8 = QAction(
            QIcon("icones/doc-save.png"), "Sauvegarder", self)
        self.action9 = QAction(QIcon("icones/doc-quit.png"), "Quitter", self)
        self.action10 = QAction(
            QIcon("icones/doc-charger.png"), "Charger", self)

        self.action_reint.setShortcut(QKeySequence("Ctrl+D"))
        ...
        self.menuGrille.addAction(self.action_reint)
        ...
        self.toolbar.addAction(self.action_reint)
...
# grille
        ...
        for i in range(9):
            for j in range(9):
                ...
                self.wg.clicked.connect(self.on_button_clicked)
...
# connexions des QAction
        self.action_reint.triggered.connect(
            self.onReset)  # reset grille de dessin
        self.action2.triggered.connect(
            self.onResetPalette)  # reset palette de couleur
...
# SAUVEGARDE
    ...
    def onSave(self):
        ...
        # Obtenir le nom du projet à partir de la QComboBox
        nom_projet = self.Hprojet.currentText()
        # Utiliser le nom du projet comme nom de fichier
        fichier = f"{nom_projet}.json"
...
# QUITTER
    def onExit(self, event):
        reply = QMessageBox.question(
            self, "Confirmation", "Êtes-vous sûr de vouloir quitter ?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
# pour la demande de choix de l'emplacement sur la palette
    def demande(self):
        QMessageBox.information(
            self, 'Information', 'vous devez choisir un emplacement dans la palette')
##################################################################
    ...
    def redFilter(self):
        ...
        for i in range(5):
            for j in range(2):
                ...
                self.wc.clicked.connect(self.apply_color)  # OK
                self.wc.clicked.connect(self.get_color2)  # OK
                self.wc.setStyleSheet(
                    f"background-color: rgb({rainbow[i][j]},0,0)")
                self.gridc.addWidget(self.wc, i, j)
    ...
    def greenFilter(self):
        ...
        for i in range(5):
            for j in range(2):
                ...
                self.wc.clicked.connect(self.get_color2)  # OK
                self.wc.setStyleSheet(
                    f"background-color: rgb(0,{rainbow[i][j]},0)")
                self.gridc.addWidget(self.wc, i, j)
    ...
    def blueFilter(self):
        ...
        for i in range(5):
            for j in range(2):
                ...
                self.wc.clicked.connect(self.get_color2)  # OK
                self.wc.setStyleSheet(
                    f"background-color: rgb(0,0,{rainbow[i][j]})")
                self.gridc.addWidget(self.wc, i, j)

##################################################################
    def on_button_clicked(self):
        index = self.H_outil.currentIndex()
        method = self.dict_button_action[index]
        method()
    ...
    def get_color2(self):
        ...
        self.Hcolordisplay.setStyleSheet(
            f"background-color: {background_color.name()};")

    def set_color2(self):
        sender_button = self.sender()
        self.selected_color = background_color = self.color_label.palette().color(
            self.color_label.backgroundRole())
    ...
    def gomme_set(self):
        ...
        # Réinitialiser le fond du bouton en blanc
        sender_button.setStyleSheet("background-color: white")
    ...
    def baguette_magique(self):
        ...
        self.color_label.setStyleSheet(
            f"background-color: {background_color_bm.name()}")
        # ligne en plus pour l'apperçu de la couleur en haut
        self.Hcolordisplay.setStyleSheet(
            f"background-color: {background_color_bm.name()};")
        for row in range(self.grid.rowCount()):
            for column in range(self.grid.columnCount()):
                ...
                # Obtenez la couleur de background du bouton
                specific_color_bm = widget.palette().color(widget.backgroundRole())
                # Vérifiez si la couleur de background correspond à la couleur spécifique souhaitée
                if specific_color_bm == background_color_bm:
                    # Mettez le fond en blanc pour les boutons ayant cette couleur
    ...
    def pot_magique(self):
        ...
        self.color_label.setStyleSheet(
            f"background-color: {background_color_p.name()}")
        # ligne en plus pour l'apperçu de la couleur en haut
        self.Hcolordisplay.setStyleSheet(
            f"background-color: {background_color_p.name()};")
        for row in range(self.grid.rowCount()):
    ...
    def select_color(self):
        self.selected_color = self.color_label.palette().color(
            self.color_label.backgroundRole())
